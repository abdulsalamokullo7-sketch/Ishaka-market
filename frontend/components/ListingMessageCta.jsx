...
export default function ListingMessageCta({ listingId, sellerUserId, sellerName }) {
  ...
  const threadHref = `/messages/${listingId}/${sellerUserId}`;
  ...
  if (!user) {
    return (
      <div className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-600">Log in to message {sellerName} about this product.</p>
        <Link href={loginRedirectUrl()} className="mt-3 inline-block rounded-full bg-brand px-4 py-2 text-sm font-semibold text-white">
          Log in to message
        </Link>
      </div>
    );

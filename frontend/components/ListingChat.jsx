...
export default function ListingChat({ listingId, sellerUserId, sellerName }) {
  ...
  const isSeller = Boolean(user?.id && sellerUserId && user.id === sellerUserId);
  ...
  }, [isSeller, sellerUserId, partners, peerUserId]);
  ...
  if (!user) {
    return (
      <div className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
        <h2 className="font-semibold text-gray-900">Chat about this listing</h2>
        <p className="mt-1 text-sm text-gray-600">Log in to message the seller about this product.</p>
        <Link
  ...
  return (
    <div className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="text-xs text-gray-500">Refreshes every few seconds</span>
      </div>
      <p className="mt-1 text-sm text-gray-600">
        {isSeller
          ? `You are the seller. Reply to buyers about “${sellerName}”.`
      ...
      {isSeller ? (
        <div className="mt-3">
          ...
          {partners.length === 0 ? (
            <p className="mt-1 text-sm text-amber-800">No messages yet. When a buyer writes you, you can pick them here.</p>
          ) : (
            <select
